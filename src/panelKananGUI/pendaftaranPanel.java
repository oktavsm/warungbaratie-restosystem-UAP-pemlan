...
public class pendaftaranPanel extends javax.swing.JPanel {
    ...
    private final String ID_PLACEHOLDER = "ID Pesanan (Auto-generated if empty)";
    private final String NAMA_PLACEHOLDER = "Masukkan Nama Pelanggan";


    public pendaftaranPanel(AntrianService antrianService, List<Produk> daftarProduk, MainFrame mainFrame) {
        this.antrianService = antrianService;
        this.daftarProduk = daftarProduk;
        this.mainFrame = mainFrame; 
        initComponents();
        muatUlangProdukComboBox();
        setupPlaceholderText();

        
        tambahPesananButton.addActionListener(e -> tambahPesananAction());
        
        genIDButton.addActionListener(e -> generateIdAction());
    }

    public void muatUlangProdukComboBox() {
        pesananComboBox.removeAllItems();
        if (daftarProduk != null) {
            for (Produk p : daftarProduk) {
                pesananComboBox.addItem(p.getNamaProduk());
            }
        }
         if (pesananComboBox.getItemCount() == 0) {
            pesananComboBox.addItem("Belum ada produk");
        }
    }

    private void setupPlaceholderText() {
        setPlaceholder(idField, ID_PLACEHOLDER);
        setPlaceholder(namaPelField, NAMA_PLACEHOLDER);
    }
    ...
    private void generateIdAction() {
        ...
        String uniqueID = "P" + (System.currentTimeMillis() % 10000); 
        idField.setText(uniqueID);
        idField.setForeground(Color.BLACK); 
    }

    private void tambahPesananAction() {
        String idPesanan = getTextFieldValue(idField, ID_PLACEHOLDER);
        String namaPelanggan = getTextFieldValue(namaPelField, NAMA_PLACEHOLDER);
        ...
        if (namaPelanggan.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Nama pelanggan tidak boleh kosong!", "Input Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (pesananComboBox.getSelectedItem() == null || pesananComboBox.getSelectedItem().toString().equals("Belum ada produk")) {
            JOptionPane.showMessageDialog(this, "Pilih pesanan terlebih dahulu atau tambahkan produk baru.", "Input Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        String pesananDipilih = pesananComboBox.getSelectedItem().toString();

        try {
            
            antrianService.tambahPesananKeAntrian(idPesanan, namaPelanggan, pesananDipilih);

            JOptionPane.showMessageDialog(this, "Pelanggan '" + namaPelanggan + "' dengan ID '" + idPesanan + "' telah ditambahkan/diperbarui pesanannya.", "Sukses", JOptionPane.INFORMATION_MESSAGE);
            
            
            setPlaceholder(idField, ID_PLACEHOLDER); 
            setPlaceholder(namaPelField, NAMA_PLACEHOLDER); 
            if (pesananComboBox.getItemCount() > 0) {
                 pesananComboBox.setSelectedIndex(0);
            }
           

            
            if (mainFrame != null) {
                mainFrame.antrianDiperbarui();
            }

        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Gagal menambahkan antrian: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace(); 
        }
    }
    ...
    private void initComponents() {
        ...
        tambahPesananButton.setText("Tambah ke Antrian");
